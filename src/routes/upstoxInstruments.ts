 ...
 */
async function safeUpsertByInstrumentKey(key: string, mapped: any) {
  if (!key) throw new Error("instrument_key is required for safeUpsert");
  try {
    // atomic upsert by unique key
    await UpstoxInstrumentModel.updateOne(
      { instrument_key: key },
      { $set: mapped },
      { upsert: true }
    );
    // return the saved doc
    return UpstoxInstrumentModel.findOne({ instrument_key: key }).lean().exec();
  } catch (err: any) {
    // E11000 duplicate key race -> retry as an update (no upsert)
    if (err && (err.code === 11000 || (err.message && err.message.includes("E11000")))) {
      return UpstoxInstrumentModel.findOneAndUpdate(
        { instrument_key: key },
        { $set: mapped },
...
router.post("/sync-option", async (req, res) => {
  try {
    ...
    if (!instrument_key) return res.status(400).json({ ok: false, error: "instrument_key is required" });
  ...
  if (save) {
    ...
    if (ops.length > 0) {
      const bulkResult = await UpstoxInstrumentModel.bulkWrite(ops, {
        ordered: false,
      });
      log.info("bulkWrite saved instruments", {
        count: ops.length,
        result: bulkResult,
      });
    }
  ...
  return res.json({
    ...
    log.error("sync-option error", err?.message || err);
...
router.get("/list", async (req, res) => {
  try {
    const { q, exchange, segment, limit = "50", skip = "0" } = req.query as any;
    const filter: any = {};
    if (q) {
      filter.$or = [
    ...
    const docs = await UpstoxInstrumentModel.find(filter)
      .limit(Number(limit))
      .skip(Number(skip))
      .lean()
      .exec();
    ...
    log.error("List instruments error", err);
    return res.status(500).json({ ok: false, error: err.message || err });
  }
});
...
router.get("/fetch-from-upstox", async (req, res) => {
  try {
    const { userId, q, save } = req.query as any;
    ...
    if ((save === "true" || save === true) && Array.isArray(instruments) && instruments.length > 0) {
      const ops = instruments.map((ins: any) => {
        const mapped = {
      ...
      if (ops.length > 0) {
        await UpstoxInstrumentModel.bulkWrite(ops, { ordered: false });
      }
    }

    return res.json({ ok: true, count: instruments.length, data: instruments });
  } catch (err: any) {
    log.error("Fetch from Upstox error", err);
    return res.status(500).json({ ok: false, error: err.message || err });
  }
});
...
router.get("/:instrumentToken", async (req, res) => {
  try {
    ...
    if (!instrumentToken) return res.status(400).json({ ok: false, error: "instrumentToken required" });

    // Try DB first
    const doc = await UpstoxInstrumentModel.findOne({ instrument_token: instrumentToken }).lean().exec();
    if (doc) {
    ...
    log.error("Get instrument error", err);
    return res.status(500).json({ ok: false, error: err.message || err });
  }
});
