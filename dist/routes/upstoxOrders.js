...
router.post("/place-option", async (req, res) => {
    try {
        ...
        logger_1.log.debug("place-option rawResp length/case", { instrument_key, hasData: !!data });
        if (!data) {
        ...
        const normalized = candidates.map((it) => {
            const raw = it.raw ?? it;
        ...
        if (!picked || !picked.instrument_token) {
            return res.status(400).json({
                ...
                debug: {
                    ...
                    sampleCandidates: normalized.slice(0, 6).map((n) => ({
        ...
        logger_1.log.error("place-option error", err?.message || err);
