...
router.get("/url", (req, res) => {
    ...
    logger_1.log.debug("Generated Upstox auth URL for state:", state);
    return res.json({
        ok: true,
        url,
        state
...
router.get("/callback", async (req, res) => {
    ...
    logger_1.log.debug("Upstox callback received:", { code: code ? "present" : "missing", state });
    ...
    try {
        ...
        logger_1.log.info("Upstox login successful for user:", tokenResp.user_id);
        ...
        logger_1.log.error("Upstox callback error:", err.message || err);
...
router.post("/refresh", async (req, res) => {
    ...
    try {
        ...
        const existingToken = await UpstoxTokens_1.default.findOne({ userId });
        ...
        logger_1.log.error("Token refresh error:", err.message || err);
        return res.status(500).json({
            ok: false,
...
router.get("/profile", async (req, res) => {
    ...
    try {
        const tokenDoc = await UpstoxTokens_1.default.findOne({ userId });
        if (!tokenDoc) {
            return res.status(404).json({
                ok: false,
        ...
        logger_1.log.error("Get profile error:", err.message || err);
        return res.status(500).json({
            ok: false,
...
router.get("/status", async (req, res) => {
    ...
    try {
        ...
        logger_1.log.error("Status check error:", err.message || err);
        return res.status(500).json({
            ok: false,
...
router.post("/logout", async (req, res) => {
    ...
    try {
        await UpstoxTokens_1.default.deleteOne({ userId });
        logger_1.log.info("User logged out:", userId);
        return res.json({
            ok: true,
        ...
        logger_1.log.error("Logout error:", err.message || err);
        return res.status(500).json({
            ok: false,
